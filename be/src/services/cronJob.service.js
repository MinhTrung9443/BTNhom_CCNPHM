...
export const scheduleOrderAutoConfirm = () => {
  ...
  cron.schedule(
    "* * * * *",
    async () => {
      logger.info("Bắt đầu kiểm tra đơn hàng cần xử lý tự động");
      const result = await orderService.autoConfirmOrders();

      if (result.confirmedCount > 0) {
        logger.info(`Cron job: Đã tự động xác nhận ${result.confirmedCount} đơn hàng COD`);
      }

      if (result.cancelledCount > 0) {
        logger.info(`Cron job: Đã tự động hủy ${result.cancelledCount} đơn hàng MoMo chưa thanh toán`);
      }
    },
    {
      scheduled: true,
  ...
  logger.info("Đã lên lịch cron job xử lý đơn hàng tự động (chạy mỗi phút)");
};
