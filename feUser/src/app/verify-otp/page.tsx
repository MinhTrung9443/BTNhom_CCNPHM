          ...
          <CardContent className="pt-6">
            <Alert variant="destructive">
              <AlertDescription>
                ...
                <Link href="/register" className="font-bold underline ml-2">Quay lại</Link>
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-green-100 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Một mã OTP gồm 6 chữ số đã được gửi đến <span className="font-bold text-gray-800">{email}</span>.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button type="submit" className="w-full bg-green-600 hover:bg-green-700" disabled={isLoading}>
              {isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Đang xác thực...</> : 'Xác thực'}
            </Button>
