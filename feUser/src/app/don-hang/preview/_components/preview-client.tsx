            ...
            <CardContent>
              <div className="space-y-3 mb-6">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(previewData?.subtotal ?? 0)}</span>
                </div>
                 <div className="flex justify-between">
                  <span className="text-gray-600">Phí vận chuyển:</span>
                  <span className="font-semibold">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(previewData?.shippingFee ?? 0)}</span>
                </div>
              ...
              <Button onClick={handleCreateOrder} className="w-full bg-green-600 hover:bg-green-700" disabled={isCreatingOrder}>
                {isCreatingOrder ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Đang xử lý...</> : 'Xác nhận và đặt hàng'}
              </Button>
            </CardContent>
          </Card>
        </div>
